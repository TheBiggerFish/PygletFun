...
cursor_text = pyglet.text.Label('', font_name='Courier', font_size=12,
                                x=WINDOW_SIZE[0]//2, y=WINDOW_SIZE[1]//2, anchor_x='left', anchor_y='center', batch=batch, color=(0, 0, 0, 255))
zoom_text = pyglet.text.Label('Zoom: 1x', font_name='Courier New', font_size=12,
                              x=10, y=WINDOW_SIZE[1]-15, batch=batch, color=(0, 0, 0, 255))
color_buffer = [[(0, 0, 0) for x in range(PIXEL_RANGE[0])]
...
for y in range(WINDOW_SIZE[1]//PIXEL_SIZE[1]):
    ...
    for x in range(WINDOW_SIZE[0]//PIXEL_SIZE[0]):
        row.append(pyglet.shapes.Rectangle(
            x*PIXEL_SIZE[0], y*PIXEL_SIZE[1], PIXEL_SIZE[0], PIXEL_SIZE[1], (0, 0, 0), batch=batch))
    rects.append(row)
...
def on_mouse_motion(x, y, dx, dy):
    cursor_text.x = x+5
    cursor_text.y = y
    text_x = (offset[0] + x/zoom_level - (WINDOW_SIZE[0] //
              PIXEL_SIZE[0])/2/zoom_level + SEED)/FREQ
    text_y = (offset[1] + y/zoom_level - (WINDOW_SIZE[1] //
              PIXEL_SIZE[1])/2/zoom_level + SEED)/FREQ
