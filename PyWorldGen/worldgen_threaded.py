...
for y in range(PIXEL_RANGE[1]):
    ...
    for x in range(PIXEL_RANGE[0]):
        row.append(pyglet.shapes.Rectangle(
            x*PIXEL_SIZE[0], y*PIXEL_SIZE[1], PIXEL_SIZE[0], PIXEL_SIZE[1], (0, 0, 0), batch))
    rects.append(row)
...
def thread_update(thread_id):
    for y in range(thread_id, PIXEL_RANGE[1], THREADS):
        for x in range(PIXEL_RANGE[0]):
            ...
            sample_x = offset[0]/FREQ + x/FREQ/zoom_level - \
                (WINDOW_SIZE[0]//PIXEL_SIZE[0])/2/FREQ/zoom_level + SEED
            sample_y = offset[1]/FREQ + y/FREQ/zoom_level - \
                (WINDOW_SIZE[1]//PIXEL_SIZE[1])/2/FREQ/zoom_level + SEED
