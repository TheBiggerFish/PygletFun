...
new_opacity = np.zeros(PIXEL_COUNTS[0]*PIXEL_COUNTS[1])
pixels = []
slimes = []
...
def initialize_threading():
    draw_threads.append(threading.Thread(target=set_opacity,args=[0,0,PIXEL_COUNTS[0]//2,0,PIXEL_COUNTS[1]//2],name='t0',daemon=True))
    draw_threads.append(threading.Thread(target=set_opacity,args=[1,PIXEL_COUNTS[0]//2,PIXEL_COUNTS[0],0,PIXEL_COUNTS[1]//2],name='t1',daemon=True))
    draw_threads.append(threading.Thread(target=set_opacity,args=[2,0,PIXEL_COUNTS[0]//2,PIXEL_COUNTS[1]//2,PIXEL_COUNTS[1]],name='t2',daemon=True))
    draw_threads.append(threading.Thread(target=set_opacity,args=[3,PIXEL_COUNTS[0]//2,PIXEL_COUNTS[0],PIXEL_COUNTS[1]//2,PIXEL_COUNTS[1]],name='t3',daemon=True))
    for i in range(len(draw_threads)):
        draw_start_locks.append(threading.Lock())
        draw_start_locks[-1].acquire()
        draw_wait_locks.append(threading.Lock())

    for t in draw_threads:
        t.start()


def set_opacity(thread_id,x_min,x_max,y_min,y_max):
    while True:
        draw_start_locks[thread_id].acquire()
        ...
        draw_wait_locks[thread_id].release()
...
def update(_):
    ...
    for l in draw_wait_locks:
        l.acquire()
    batch.draw()

    slimes_pos = np.array(slimes).flatten()
    new_opacity = np.zeros(len(opacity_buffer))
    draw[GPU_BLOCKS_SHAPE,GPU_THREADS_SHAPE](opacity_buffer, slimes_pos, new_opacity)

    # print(opacity_buffer[0:4],new_opacity[0:4])
